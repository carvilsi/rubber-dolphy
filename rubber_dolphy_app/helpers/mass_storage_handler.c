...
#define TAG "MASS_STORAGE_HANDLER"
...
static bool file_read(
    ...
    *out_len = storage_file_read(app->file, out, clamp);
    FURI_LOG_T(TAG, "READ: %lu/%lu", *out_len, count * SCSI_BLOCK_SIZE);
...
void mass_storage_init(void* context) {
    ...
    app->usb = mass_storage_usb_start(app->img_path, fn);
}

void mass_storage_end(void* context) {
    BadUsbApp* app = context;
